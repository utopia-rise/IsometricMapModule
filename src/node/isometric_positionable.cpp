...
IsometricPositionable::IsometricPositionable() :
		Node2D(), debug_points(), aabb({ 0, 0, 0 }, { 1, 1, 1 }), z_order_size(0), rendered(false), is_temporary(true), debug_z(0), outline_drawer(nullptr), world(nullptr), world_owner(false) {
	on_resize();
}
...
void IsometricPositionable::_enter_tree() {
	...
	if (world != RID() && world_owner) {
		IsometricServer::get_instance()->unregister_isometric_element(self, world);
		world_owner = false;
	}
	self = RID();
	world = RID();
	...
	if (parent) {
		if (auto positionable{ cast_to<IsometricPositionable>(parent) }) {
			if (positionable->world != RID() && !is_temporary) {
				world = positionable->world;
				world_owner = false;
				aabb.position = local_position + positionable->aabb.position;
				IsometricServer::get_instance()->register_isometric_element(this, world, is_dynamic);
				return;
			}
		}
	}
	world = IsometricServer::get_instance()->create_world();
	world_owner = true;
}
...
void IsometricPositionable::set_global_position_3d(Vector3 pos) {
	...
	set_position(resource::IsometricConfiguration::getDefaultConfiguration()->get_screen_coord_from_3d(get_local_position_3d()));
}
...
void IsometricPositionable::prepare_points() {
	...
	if (deb_z > -1) {
		Vector2 gridOffset(0, -resource::IsometricConfiguration::getDefaultConfiguration()->get_e_z() * static_cast<real_t>(deb_z));
