...
static resource::PlaceholderType *default_placeholder_type{ nullptr };

IsometricPlaceholder::IsometricPlaceholder() : IsometricElement(),
		left_color(),
		right_color(),
		up_color(),
		down_color(),
...
Ref<resource::PlaceholderType> IsometricPlaceholder::get_placeholder_type() const {
	return placeholder_type;
}

void IsometricPlaceholder::set_placeholder_type(Ref<resource::PlaceholderType> p_type) {
	if (p_type.is_valid()) {
		placeholder_type = p_type;
	} else {
		if (unlikely(!default_placeholder_type)) {
			default_placeholder_type = memnew(resource::PlaceholderType());
			default_placeholder_type->set_color(Color(1, 1, 1, 1));
			default_placeholder_type->set_type_name("default");
		}
		placeholder_type = Ref<resource::PlaceholderType>(default_placeholder_type);
	}
	update_colors();
	update();
}
...
void IsometricPlaceholder::draw_points() {
	...
	if (debugZ >= 0) {
		...
		real_t zRatio{ resource::IsometricConfiguration::getDefaultConfiguration()->get_z_ratio() };
		int tileWidth{ resource::IsometricConfiguration::getDefaultConfiguration()->get_tile_width() };
		int tileHeight{ resource::IsometricConfiguration::getDefaultConfiguration()->get_tile_height() };
		float eZ{ resource::IsometricConfiguration::getDefaultConfiguration()->get_e_z() };
...
void IsometricPlaceholder::_bind_methods() {
	...
	ADD_PROPERTY_DEFAULT("placeholder_type", Ref<resource::PlaceholderType>(nullptr));
}
