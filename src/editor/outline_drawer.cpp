...
void OutlineDrawer::draw_outline(node::IsometricPositionable* positionable) {
    ...
    RenderingServer::get_singleton()->canvas_item_clear(outline_rid);
    RenderingServer::get_singleton()->canvas_item_set_parent(outline_rid, positionable->get_canvas_item());
    RenderingServer::get_singleton()->canvas_item_set_z_as_relative_to_parent(outline_rid, true);

    if (const data::IsometricParameters
        * space_configuration {
          IsometricServer::get_instance()->space_get_configuration(positionable->get_space_RID())}) {
        PackedVector2Array points {utils::get_bounding_box(*space_configuration, positionable->get_size())};
        ...
        if (outline_data.should_draw_polygons) {
            RenderingServer::get_singleton()->canvas_item_add_polygon(outline_rid, up_points, upColor);
            RenderingServer::get_singleton()->canvas_item_add_polygon(outline_rid, leftPoints, leftColor);
            RenderingServer::get_singleton()->canvas_item_add_polygon(outline_rid, rightPoints, rightColor);
        }
        ...
        // Upper Lines
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, up_points[0], up_points[1], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, up_points[1], up_points[2], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, up_points[2], up_points[3], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, up_points[3], up_points[0], color, line_size);

        // Vertical Lines

        // draw_line(up_p[0], down_p[0], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, up_points[1], down_points[1], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, up_points[2], down_points[2], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, up_points[3], down_points[3], color, line_size);

        // Lower Lines
        // draw_line(down_p[0], down_p[1], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, down_points[1], down_points[2], color, line_size);
        RenderingServer::get_singleton()->canvas_item_add_line(outline_rid, down_points[2], down_points[3], color, line_size);
        // draw_line(down_p[3], down_p[0], color, line_size);

        RenderingServer::get_singleton()->canvas_item_set_visible(outline_rid, outline_data.is_visible);
    }
}
...
void OutlineDrawer::set_outline_visible(node::IsometricPositionable* positionable, bool visible) {
    ...
    RenderingServer::get_singleton()->canvas_item_set_visible(outline_data.rid, visible);
}
