...
void EditorUtils::refresh_item_list_containing_tiles_for_category(const String& category, ItemList* item_list,
    ...
    for (int i = 0; i < scene_to_add_size; ++i) {
        ...
        PositionableScenesCacheManager::get_instance().add_scene(
                calling_control,
    ...
    for (int i = 0; i < removed_elements.size(); ++i) {
        ...
        item_list->set_item_icon(
                last_item_id,
                EditorInterface::get_singleton()->get_base_control()->get_icon("Remove", "EditorIcons")
        );
    }
}
...
void EditorUtils::refresh_positionable_icons_for_item_list(ItemList* item_list, Control* registered_in_manager) {
    for (int i = 0; i < item_list->get_item_count(); ++i) {
        ...
        item_list->set_item_icon(
                i,
                PositionableScenesCacheManager::get_instance().get_icon(registered_in_manager, i)
        );
    }
}
